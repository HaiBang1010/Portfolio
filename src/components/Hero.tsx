...
export default function Hero() {
  ...
  return (
    <section className="w-full flex flex-col items-start pt-20">
      ...
      >
        <h2 className="text-base md:text-lg font-bold tracking-widest uppercase text-zinc-500 mb-8 font-mono">
          Tech Stack
        </h2>
        <div className="grid grid-cols-3 md:grid-cols-5 gap-6">
          {skills.map((skill) => (
            <motion.div
              key={skill.name}
              className="flex flex-col items-center justify-center p-3 rounded-xl border border-zinc-800 bg-zinc-900/50 card-hover"
            >
              <div className="w-12 h-12 mb-3 flex items-center justify-center">
                {skill.icon ? (
                  ...
                  <div className="w-12 h-12 rounded bg-zinc-800 flex items-center justify-center text-sm font-mono text-zinc-500">
                    {skill.name.charAt(0)}
                  </div>
                )}
              </div>
              <span className="text-xs md:text-base text-zinc-300">{skill.name}</span>
            </motion.div>
          ))}
        </div>
      </motion.div>
